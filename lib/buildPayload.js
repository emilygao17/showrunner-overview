...
/**
 * Fetch all matching rows from a table, paging in batches of 3.
 * Batching prevents overwhelming Supabase's connection pool when tables
 * have many pages (e.g. shows = 15 pages).
...
async function fetchAll(tableName, selectFields, filterFn = null) {
  ...
  // Step 2: pages in batches of BATCH_SIZE to avoid connection pool exhaustion
  const totalPages = Math.ceil(count / PAGE_SIZE);
  const all = [];
  for (let i = 0; i < totalPages; i += BATCH_SIZE) {
    const batch = Array.from(
      { length: Math.min(BATCH_SIZE, totalPages - i) },
      (_, j) => withFilter(query(tableName).select(selectFields))
        .range((i + j) * PAGE_SIZE, (i + j + 1) * PAGE_SIZE - 1)
    );
...
export async function loadAllData(ym) {
  ...
  const [venuesRaw, showsRaw, seenRaw, wtsRaw, viewsRaw, listsRaw] = await Promise.all([
    fetchAll('venues',     'venue_id, venue_name, neighborhood, city, latitude, longitude, gallery_stage_name, venue_type'),
    fetchAll('shows',      'show_id, show_title, display_title, venue_id, start_date, end_date, show_medium, show_subject_matter, image_urls, short_description, artist_names, artist_count'),
...
// ── Available months ─────────────────────────────────────────────────────────
// Queries the min/max created_at across activity tables and returns every
// YYYY-MM in that range (inclusive). New months appear automatically as data
// arrives — no code changes needed when a new month rolls over.

const ACTIVITY_TABLES = ['seen_shows', 'show_views', 'want_to_see', 'saved_lists'];
let _monthsCache = null;
let _monthsCacheTs = 0;

export async function getAvailableMonths() {
  if (_monthsCache && Date.now() - _monthsCacheTs < CACHE_TTL) return _monthsCache;

  const probes = ACTIVITY_TABLES.flatMap(t => [
    query(t).select('created_at').order('created_at', { ascending: true  }).limit(1),
    query(t).select('created_at').order('created_at', { ascending: false }).limit(1),
  ]);
  const results = await Promise.all(probes);

  const dates = [];
  for (const { data, error } of results) {
    if (error) throw new Error(`Supabase error fetching month bounds: ${error.message}`);
    for (const r of (data || [])) if (r.created_at) dates.push(r.created_at);
  }
  if (!dates.length) {
    _monthsCache = []; _monthsCacheTs = Date.now();
    return [];
  }

  const earliest = dates.reduce((a, b) => a < b ? a : b).slice(0, 7);
  const latest   = dates.reduce((a, b) => a > b ? a : b).slice(0, 7);
  ...
  _monthsCache = months; _monthsCacheTs = Date.now();
  return months;
}
...
export function buildPayload(ym, rawData) {
  ...
  const totalOnViewShows = Object.values(shows).filter(s => {
    const startStr = (s.start_date ?? '').trim();
    const endStr   = (s.end_date   ?? '').trim();
  ...
  // Snapshot — "by the numbers" stats
  const openedThisMonth = Object.values(shows).filter(s => {
    const startStr = (s.start_date ?? '').trim();
    if (!startStr) return false;
    const startD = new Date(startStr + 'T00:00:00');
    if (startD < firstDay || startD > lastDay) return false;
    const title = s.display_title || s.show_title;
    return !title.toLowerCase().includes('title tbd');
  });
  const snapshot = {
    total_seen: checkins.length,
    shows_opened:      openedThisMonth.length,
    museum_openings:   openedThisMonth.filter(s => venues[s.venue_id]?.venue_type === 'Museum').length,
    gallery_openings:  openedThisMonth.filter(s => venues[s.venue_id]?.venue_type === 'Gallery').length,
    solo_shows:        openedThisMonth.filter(s => Number(s.artist_count) === 1).length,
    group_shows:       openedThisMonth.filter(s => Number(s.artist_count) > 1).length,
  };
  ...
  return {
    ...
    on_view_stats:         { medium: onViewMedium, subject: onViewSubject, total: totalOnViewShows },
    snapshot,
